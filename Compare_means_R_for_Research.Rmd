...
Null hypothesis will be one of the following form
...
Corresponding alternative hypothesis are:
...
1. for equality null hypothesis:
...
```
 ...
 Since the p- value is greater that accepted level of significance (0.05), there is no reason to reject the null hypothesis that *the data is normally distributed*.
...
3. for greater than or equal to null hypothesis:
<center>
...
- whether the mean of group A ($\mu_1$) is greater than the mean of group B ($\mu_2$)?
...
- For testing of equality of means *independence, normality and same variability* should be ensured.
...
Let us use the same example in the case of two sample t- test for convenience.
...
- whether the mean difference ($d$) is greater than 0?
